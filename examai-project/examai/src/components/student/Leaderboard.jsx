...
export default function Leaderboard() {
  ...
  var [selectedExam, setSelectedExam] = useState('global');
  var [leaders, setLeaders] = useState([]);
  var [loading, setLoading] = useState(true);

  useEffect(function() {
    loadGlobal();
    store.loadExams().then(function(d) { setExams(d); }).catch(function(){});
  }, []); // eslint-disable-line

  function loadGlobal() {
    setLoading(true); setSelectedExam('global');
    store.loadGlobalLeaderboard().then(function(d) { setLeaders(d||[]); setLoading(false); }).catch(function() {
      // fallback: aggregate from all exams
      setLeaders([]); setLoading(false);
    });
  }

  function loadExamBoard(eid) {
    setLoading(true); setSelectedExam(eid);
  ...
  return (
    <div className="fade-up">
      ...
      <div className="filter-pills" style={{ marginBottom: 20 }}>
        <button className={'pill' + (selectedExam==='global' ? ' active' : '')} onClick={loadGlobal}>🌐 Global</button>
        {exams.map(function(e) {
          return <button key={e.exam_id} className={'pill' + (selectedExam===e.exam_id ? ' active' : '')} onClick={function() { loadExamBoard(e.exam_id); }}>{e.title}</button>;
        })}
      </div>
      {loading ? <div className="loading-center"><div className="spinner"></div></div>
        : leaders.length === 0 ? <div className="empty-state"><div className="empty-state-icon">🏅</div><div className="empty-state-title">No submissions yet</div></div>
        : (
        <div className="card">
          <table style={{ width:'100%', borderCollapse:'collapse' }}>
            <thead>
              <tr style={{ borderBottom:'2px solid var(--border)' }}>
                ...
                {selectedExam === 'global' && <th style={{ textAlign:'center', padding:'12px 8px', fontSize:'0.8rem', color:'var(--text3)', fontFamily:'JetBrains Mono,monospace', textTransform:'uppercase' }}>Exams</th>}
              ...
              {leaders.map(function(l, i) {
                return (
                  <tr key={i} style={{ borderBottom:'1px solid var(--border)', background:i<3?'rgba(124,58,237,0.03)':'transparent' }}>
                    <td style={{ padding:'12px 8px', fontWeight:700, fontSize:'1.1rem' }}>{medal(i)}</td>
                    ...
                    {selectedExam === 'global' && <td style={{ padding:'12px 8px', textAlign:'center', fontSize:'0.85rem', color:'var(--text3)' }}>{l.exam_count||'-'}</td>}
                    <td style={{ padding:'12px 8px', textAlign:'center', fontWeight:700, fontFamily:'JetBrains Mono,monospace' }}>{Number(l.avg_score||l.total_score||0).toFixed(2)}</td>
