...
router.post('/', requireAdmin, async(req, res) => {
    try {
        ...
        const [result] = await pool.query(
            'INSERT INTO exams (title, description, duration_minutes, total_marks, scheduled_at, end_at, created_by, status) VALUES (?, ?, ?, ?, ?, ?, ?, "draft")', [title, description, duration_minutes, total_marks, scheduled_at || null, end_at || null, req.user.user_id]
        );
...
router.put('/:id', requireAdmin, async(req, res) => {
    try {
        const { title, description, duration_minutes, total_marks, scheduled_at, end_at } = req.body;
        await pool.query(
            'UPDATE exams SET title=?, description=?, duration_minutes=?, total_marks=?, scheduled_at=?, end_at=? WHERE exam_id=? AND created_by=?', [title, description, duration_minutes, total_marks, scheduled_at || null, end_at || null, req.params.id, req.user.user_id]
        );
...
router.post('/:id/publish', requireAdmin, async(req, res) => {
    try {
        const [
            [{ qCount }]
        ] = await pool.query('SELECT COUNT(*) as qCount FROM questions WHERE exam_id=?', [req.params.id]);
...
router.get('/:id/export-csv', requireAdmin, async(req, res) => {
    try {
        ...
        const [
            ...
            'SELECT * FROM exams WHERE exam_id = ? AND created_by = ?', [examId, adminId]
        );
        if (!exam) return res.status(404).json({ error: 'Exam not found' });

        // Get all questions for this exam
        const [questions] = await pool.query(
            'SELECT question_id, question_text, question_type, marks, correct_answer FROM questions WHERE exam_id = ? ORDER BY question_id', [examId]
        );
    ...
    `, [examId]);
        ...
        submissions.forEach(function(s, idx) {
            ...
            questions.forEach(function(q) {
                ...
                if (!ans || ans === '') { unanswered++; } else if (['MCQ', 'TRUE_FALSE'].includes(q.question_type)) {
